
export const projects = [
  
  {
    projectName: "FAKESTORE ",
    image:"projects/fake.png",
    type: "React",
    description: "Features include product display, cart functionality, Wishlist management, and order placement.",
    link: "http://mr-unick.github.io/fakestore",
    technologies: ["API", "React.js", "Redux", "Axios", "HTML", "CSS","Frontend"],
    
  },
  {
    projectName: "PORTFOLIO",
    image:"projects/port.png",
    type: "React",
    description: "Technologies used include React.js, HTML, and CSS.",
    link: "http://mr-unick.github.io/portfolio",
    technologies: ["React", "HTML", "CSS","Frontend"],
   
  },
  {
    projectName: "Food Delivery App",
    type: "Android",
    image:"projects/food.png",
    description: "Technologies include React Native, Redux, and Tailwind CSS.",
    link: "https://example.com/food-delivery",
    technologies: ["React Native", "Redux", "Tailwind CSS","Android"],
  
  },
  {
    projectName: "Name-it ",
    type: "React",
    image:"projects/name.png",
    description: "Technologies used are HTML, CSS, and React.",
    link: "https://mr-unick.github.io/name-it/",
    technologies: ["HTML", "CSS", "React","Frontend"],
   
  },
  {
    projectName: "Crypto Tracker",
    type: "Frontend",
    image:"projects/crypto.png",
    description: "The project utilizes HTML, CSS, and JavaScript for development.",
    link: "https://mr-unick.github.io/crypto/",
    technologies: ["HTML", "CSS", "JavaScript","Frontend"],
   
  },
  {
    projectName: "Weather App",
    type: "React",
    image:"projects/weather.png",
    description: "The project uses React.js, HTML, CSS, and integrates with a weather API.",
    link: "https://weather-app-orpin-delta.vercel.app/",
    technologies: [ "HTML", "CSS","React","Frontend"],
    

  },
  {
    projectName: "Login System",
    type: "MERN",
    image:"projects/login.png",
    description: "The project uses React.js, HTML, CSS, and integrates with a weather API.",
    link: "https://login-client-brown.vercel.app/login",
    technologies: [ "HTML", "CSS","MERN",,"React"],
   
  },
  {
    projectName: "Chat App",
    type: "MERN",
    image:"projects/chat.png",
    description: "The project uses React.js, HTML, CSS, and integrates with a weather API.",
    link: "https://login-client-brown.vercel.app/login",
    technologies: [ "HTML", "CSS","MERN","socket.io","React"],
    

  },
  {
    projectName: "FastType",
    type: "React",
    image:"projects/fast.png",
    description: "The project uses React.js, HTML, CSS, and integrates with a weather API.",
    link: "https://mr-unick.github.io/flash-type/",
    technologies: [ "HTML", "CSS","React","Frontend"],
    

  },
  {
    projectName: "DEVSPACE",
    type: "MERN",
    image:"projects/devspace.png",
    description: "Technologies used include MongoDB, Express.js, Ejs, Node.js, HTML, and CSS.",
    link: "https://example.com/devspace",
    technologies: ["MongoDB", "Express.js", "Ejs", "Node.js", "HTML", "CSS","MERN"],
   
  },
  {
    projectName: "QUIZ",
    type: "React",
    image:"projects/quiz.png",
    description: "Technologies used include MongoDB, Express.js, React.js, Node.js, HTML, and CSS.",
    link: "https://example.com/notesapp",
    technologies: ["MongoDB", "Express.js", "React", "Node.js", "HTML", "CSS","Frontend"],
  
  },
  {
    projectName: "NOTESAPP",
    type: "Mern",
    image:"projects/fake.png",
    description: "Technologies used include MongoDB, Express.js, React.js, Node.js, HTML, and CSS.",
    link: "https://example.com/notesapp",
    technologies: ["MongoDB", "Express.js", "React", "Node.js", "HTML", "CSS","MERN"],
  
  },
];

  
  export const intro =[
    {
      "role": "Web Content Writer",
      "description": "Responsible for creating and editing web content, including articles, blog posts, and product descriptions. Ensures content is engaging, informative, and SEO-friendly.",
      "image":  "images/web.jpg"
    },
    {
      "role": "React Native Developer",
      "description": "Specializes in building Android Applications using the React Native. Develops reusable components, manages state, and integrates with backend services.",
      "image": "images/android.jpg"
    },
    {
      "role": "Web Designer",
      "description": "Creates visually appealing and user-friendly web designs. Works closely with frontend developers to ensure designs are implemented effectively.",
      "image": "images/uiux.jpg"
    },
    {
      "role": "MERN Stack Developer",
      "description": "Develops full-stack web applications using the MERN (MongoDB, Express, React, Node.js) stack. Manages databases, server, and client-side logic.",
      "image": "images/webdev.jpg"
    }
  ]
  
  export const Skillsdata=[
    {
        "categoryName": "Core languages",
        "skills": [
            {
                "skillName": "C",
                "skillData": "skills/cc.svg"
            },
            {
                "skillName": "C++",
                "skillData": "skills/cpp.svg"
            },
            {
                "skillName": "Java",
                "skillData": "skills/java.svg"
            },
            {
                "skillName": "JavaScript",
                "skillData": "skills/js.svg"
            },
            {
                "skillName": "Python",
                "skillData": "skills/py.svg"
            }
        ]
    },
    {
        "categoryName": "Frontend Technologies",
        "skills": [
            {
                "skillName": "HTML5",
                "skillData": "skills/html.svg"
            },
            {
                "skillName": "CSS",
                "skillData": "skills/css.svg"
            },
            {
                "skillName": "Tailwind CSS",
                "skillData": "skills/tailwindcss-icon.svg"
            },
            {
                "skillName": "Material UI",
                "skillData": "skills/ui.svg"
            },
            {
                "skillName": "React",
                "skillData": "skills/rec.svg"
            },
            {
                "skillName": "Redux",
                "skillData": "skills/redux.svg"
            },
            {
              "skillName": "NextJs",
              "skillData": "skills/next.svg"
            },
            {
              "skillName": "React Native",
              "skillData": "skills/native.svg"
            }
        ]
    },
    {
        "categoryName": "BackEnd Technologies",
        "skills": [
            {
                "skillName": "Node.js",
                "skillData": "skills/no.svg"
            },
            {
                "skillName": "Express.js",
                "skillData": "skills/exx.svg"
            },
            {
                "skillName": "Socket.io",
                "skillData": "skills/socketio-icon.svg"
            }
        ]
    },
    {
        "categoryName": "Databases",
        "skills": [
            {
                "skillName": "MongoDB",
                "skillData": "skills/mon.svg"
            },
            {
                "skillName": "MySQL",
                "skillData": "skills/mysql-icon.svg"
            }
        ]
    }
]



